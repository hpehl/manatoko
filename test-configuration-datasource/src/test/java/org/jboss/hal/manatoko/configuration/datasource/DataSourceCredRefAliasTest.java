...
class DataSourceCredRefAliasTest extends DataSourceCredRefBase {

    /**
     * Update the alias and clear-text value of the credential reference. The number of aliases in the credential store must
     * increase by one.
     */
    @Test
    void updateAlias() throws Exception {
        String newAliasValue = Random.name();
        form.edit();
        form.text(ALIAS, newAliasValue);
        ...
        new ResourceVerifier(credentialStoreAddress(CRED_ST_UPDATE), client).verifyTrue(
                ...
                () -> {
                    ...
                    return assertAlias(result, newAliasValue);
                });
    }
}
