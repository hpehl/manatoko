...
import static org.jboss.hal.testsuite.fixtures.MessagingFixtures.JGROUPS_DISCOVERY_GROUP_ITEM;
...
class JgroupsDiscoveryGroupTest extends AbstractClusteringTest {
    ...
    static void setupModel() throws Exception {
        ...
        operations.add(jgroupsDiscoveryGroupAddress(SRV_UPDATE, JGROUPS_DG_UPDATE),
                Values.of(JGROUPS_CLUSTER, Random.name()));
        operations.add(jgroupsDiscoveryGroupAddress(SRV_UPDATE, JGROUPS_DG_DELETE),
                Values.of(JGROUPS_CLUSTER, Random.name()));
    }
    ...
    void setUp() {
        ...
        console.verticalNavigation().selectSecondary(DISCOVERY_GROUP_ITEM, JGROUPS_DISCOVERY_GROUP_ITEM);
        table = page.getJgroupsDiscoveryGroupTable();
        form = page.getJgroupsDiscoveryGroupForm();
        table.bind(form);
    }

    @Test
    void create() throws Exception {
        crudOperations.create(jgroupsDiscoveryGroupAddress(SRV_UPDATE, JGROUPS_DG_CREATE), table, f -> {
            f.text(NAME, JGROUPS_DG_CREATE);
