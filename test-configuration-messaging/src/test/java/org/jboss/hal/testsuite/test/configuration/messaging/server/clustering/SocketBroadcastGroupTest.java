...
import static org.jboss.hal.testsuite.fixtures.MessagingFixtures.SOCKET_BROADCAST_GROUP_ITEM;
...
class SocketBroadcastGroupTest extends AbstractClusteringTest {
    ...
    static void setupModel() throws Exception {
        ...
        operations.add(socketBroadcastGroupAddress(SRV_UPDATE, SOCKET_BG_UPDATE),
                Values.of(SOCKET_BINDING, Random.name()).and(CONNECTORS, connectors));
        operations.add(socketBroadcastGroupAddress(SRV_UPDATE, SOCKET_BG_DELETE),
                Values.of(SOCKET_BINDING, Random.name()).and(CONNECTORS, connectors));
    }
    ...
    void prepare() {
        ...
        console.verticalNavigation().selectSecondary(BROADCAST_GROUP_ITEM, SOCKET_BROADCAST_GROUP_ITEM);
        table = page.getSocketBroadcastGroupTable();
        form = page.getSocketBroadcastGroupForm();
        table.bind(form);
    }

    @Test
    void create() throws Exception {
        crudOperations.create(socketBroadcastGroupAddress(SRV_UPDATE, SOCKET_BG_CREATE), table, f -> {
            f.text(NAME, SOCKET_BG_CREATE);
