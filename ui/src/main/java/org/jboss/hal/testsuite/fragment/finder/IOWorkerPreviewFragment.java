...
public class IOWorkerPreviewFragment {
    ...
    @FindByJQuery("h2:contains('Connections'):visible ~ ul." + listGroup + " > li") private List<WebElement> connections;

    @FindByJQuery("div.progress-description[title='Core Pool Size'] + div") private WebElement corePoolSize;

    @FindByJQuery("div.progress-description[title='Max Pool Size'] + div") private WebElement maxPoolSize;

    @FindByJQuery("div.progress-description[title='IO Thread Count'] + div") private WebElement ioThreadCount;

    @FindByJQuery("div.progress-description[title='Busy Task Thread Count'] + div") private WebElement busyTaskThreadCount;

    @FindByJQuery("a.clickable > span.fa-refresh") private WebElement refreshButton;
